...
まず最初に、この画像がどんな書類かを documentType として判定してください:
- "estimate": 整備・修理・部品交換等の費用の見積書・請求書・注文書・領収書など、金額の内訳が書かれた書類
- "shaken_certificate": 自動車検査証(車検証)。登録番号・車台番号・所有者・有効期限等が印字された公的書類で、金額の内訳は無い
- "other": 上記のどちらでもない書類

documentType が "estimate" の場合のみ、以下を読み取ってください。
documentType が "estimate" 以外の場合、items は空配列 [] にし、shopName・estimateDate・totalAmount もnullのままにしてください
(車検証等の内容を見積の項目として無理に読み取らないこと)。
...
- oilGrade: 見積書に記載されているオイルの銘柄・グレード・粘度をそのまま(要約・分類せず)書き写してください。
  例:「化学合成油 5W-30」「鉱物油」「スーパーオイル」「カストロールGTX」など、記載通りの文言。
  種別が全く記載されていなければnull(「不明」等と補って書かないこと)
